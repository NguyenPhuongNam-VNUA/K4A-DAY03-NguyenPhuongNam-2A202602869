...
if __name__ == "__main__":
    ...
    sched_tool = next((t for t in tools if t.get("name") in ["update_order_status", "schedule_appointment"]), None)
    if sched_tool and not sched_tool.get("parameters", {}).get("properties"):
        ...
        tool_name = sched_tool.get("name") if sched_tool else "update_order_status"
        print(f"✅ [TODO 1.2]: Tool '{tool_name}' đã có schema đầy đủ.")

    # Kiểm tra trạng thái TODO 2.1 (call_tool)
    test_result = server.call_tool("order_query", {"tracking_id": "VN2026_001"})
    if not test_result:
